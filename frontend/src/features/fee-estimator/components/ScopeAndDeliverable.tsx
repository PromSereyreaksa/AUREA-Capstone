...
export const ScopeAndDeliverable: React.FC<ScopeAndDeliverableProps> = ({
  ...
  const removeDeliverable = (id: string) => {
    onUpdate(deliverables.filter((item) => item.id !== id));
  };
  ...
  const getDeliverableIcon = (type: string) => {
    const predefined = deliverableTypes.find((item) => item.type === type);
    return (
      predefined?.icon || (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2">
  ...
  const canProceed =
    deliverables.length > 0 && deliverables.some((item) => item.quantity > 0);
    ...
    <div className="estimator-shell">
      ...
      <div className="fee-estimator-body">
        <div className="form-section nb-cut-in-up">
          ...
          <p className="estimator-microcopy" style={{ marginBottom: '1.5rem' }}>
            Set the quantity for each deliverable the project actually needs.
          </p>

          <div className="estimator-card-list" style={{ marginBottom: '2rem' }}>
            {deliverables.map((item) => (
              <div key={item.id} className="deliverable-item">
                <div className="deliverable-icon">{getDeliverableIcon(item.type)}</div>
                ...
                <div className="deliverable-content">
                  ...
                  <div className="estimator-range-row">
                    ...
                    <input
                      ...
                      style={{ flex: 1 }}
                    />
                    <span className="estimator-range-label">10+</span>
                    ...
                    <div className="estimator-range-side">
                      ...
                      />
                      <span className="estimator-badge">Quantity</span>
                    </div>
                  </div>
                </div>
                ...
                {item.isCustom && (
                  <button
                    ...
                    className="estimator-remove-btn"
                    title="Remove item"
                  >
                    X
                  </button>
          ...
          <div className="estimator-panel estimator-panel-muted">
            <p className="estimator-eyebrow">Custom Deliverable</p>
            <div className="estimator-add-row">
              <input
                type="text"
            ...
            <p className="estimator-microcopy">
              Add anything missing from the default list, then adjust the quantity before continuing.
            </p>
          </div>
        </div>
